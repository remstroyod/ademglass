...
?>
<div class="catalog__filter-cell  <?php echo berocket_isset($css_class) ?> berocket_aapf_widget-wrapper <?php echo "brw-" . preg_replace( "#^(pa)?_#", "", $attribute ); ?> <?php if( $is_child_parent ) echo ' br_child_parent_wrapper'; if ( ! empty($description) ) echo ' berocket_widget_has_description'; ?>">
    <?php if( $is_child ) { ?>
    ...
    <div class="berocket_aapf_widget-title_div<?php if ( ! empty($is_hide_mobile) ) echo ' berocket_aapf_hide_mobile'; if(!empty($widget_collapse_disable)) echo ' disable_collapse';?>">
        ...
        <?php if( ! empty($title) || ! empty($icon_before_title) || ! empty($icon_after_title) ) { ?>
            ...
            <h5 class="widget-title berocket_aapf_widget-title"><span><?php echo $title ?></span></h5>

        <?php } ?>
    </div>
    <ul class='form-group form-checkbox berocket_aapf_widget <?php echo ( br_widget_is_hide( $attribute, ! empty( $widget_is_hide ) ) ? 'berocket_style_none' : 'berocket_style_block' ) ?> <?php echo berocket_isset($product_count_style); ?> <?php if ( ! empty($is_hide_mobile) ) echo ' berocket_aapf_hide_mobile' ?> <?php echo berocket_isset($class) ?> <?php echo ( ( berocket_isset($type) == 'tag_cloud' ) ? 'berocket_aapf_widget-tag_cloud' : '' ) ?>
